...
var NoticeManager = class {
  constructor(plugin) {
    this.createNotice = (message, duration, color) => {
      let notice;
      if (color) {
        ...
        notice = new import_obsidian.Notice(fragment, duration);
      } else {
...
// src/main.ts
var getPublishUrl = (path, publishDomain, permalink) => {
  if (permalink)
    return `https://${publishDomain}/${permalink}`;
  const pathWithoutExtension = path.replace(/\.md$/, "");
  const encodedPath = encodeURIComponent(pathWithoutExtension).replace(/%20/g, "+").replace(/%2F/g, "/");
  return `https://${publishDomain}/${encodedPath}`;
};
function isMarkdownFile(file) {
  return file && file.extension === "md";
}
var PublishUrlSetting = class extends import_obsidian2.Plugin {
  constructor() {
    super(...arguments);
    this.eventRefs = [];
    this.copyPublishUrl = (file) => {
      var _a;
      const path = file.path;
      const frontmatter = (_a = this.app.metadataCache.getFileCache(file)) == null ? void 0 : _a.frontmatter;
      const publishDomain = this.settingManager.getSettings().publishDomain;
      const permalink = frontmatter == null ? void 0 : frontmatter.permalink;
      const publishUrl = getPublishUrl(path, publishDomain, permalink);
      return publishUrl;
    };
    this.copyTheogUrl = (url, template) => {
      const theogBaseUrl = "https://theog.io/goto";
      return `${theogBaseUrl}?url=${url}&template=${template}`;
    };
    this.createNotice = (...props) => this.noticeManager.createNotice(...props);
  }
  async onload() {
    const that = this;
    ...
    this.addCommand({
      ...
      checkCallback(checking) {
        const activeLeaf = that.app.workspace.getActiveViewOfType(import_obsidian2.MarkdownView);
        if (!activeLeaf)
          return;
    ...
    this.addCommand({
      ...
      checkCallback(checking) {
        ...
        const frontmatter = (_a = that.app.metadataCache.getFileCache(file)) == null ? void 0 : _a.frontmatter;
        const cover = frontmatter == null ? void 0 : frontmatter.cover;
...
var SettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian2.Setting(containerEl).setName("Publish domain").addText(
      (text) => text.setPlaceholder("Your publish domain").setValue(
    ...
    new import_obsidian2.Setting(containerEl).setName("Template number").setDesc("The template number of theog.io").addText((text) => {
      text.setPlaceholder(String(DEFAULT_SETTING.theogTemplate)).setValue(
        String(
